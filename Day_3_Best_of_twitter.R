...
for (j in 1:ncol(input)) {
  ...
  if (nrow(co2) > 1) {
    ...
    co2 <- co2[co2[, j] != common[j], ]  #uses that ! to inverse the 0/1 again.
  }
}
binary_to_int(oxygen) * binary_to_int(co2)


## and one more from @antoine_fabri

input <- scan("./Day_3/Part_1/input.txt", what = character(), sep = "", quiet = TRUE)
mat   <- do.call(rbind, strsplit(input, "")) == 1  ## makes 0 /1 true/false

# part 1
gamma <- colMeans(mat) >= .5
epsilon <- !gamma
powers <- 2^(rev(seq_along(gamma)-1))
sum(powers * gamma) * sum(powers * epsilon)
...
sum(powers * oxygen) * sum(powers * co2)
